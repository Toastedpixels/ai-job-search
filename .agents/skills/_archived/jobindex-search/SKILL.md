...
> **ARCHIVED:** Inactive for the current US-market search configuration. Moved out of `.agents/skills/*/SKILL.md` glob scope so `/scrape` no longer auto-discovers it. Kept for reference and in case Danish-market search is needed again — to reactivate, move this folder back to `.agents/skills/jobindex-search/` and update `allowed-tools` path references above.
