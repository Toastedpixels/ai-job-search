...
- **No tables anywhere in Professional Experience or Education.** Each entry uses the `\cvheader{Title}{Company, Location}{Dates}` command: `Title -- Dates` on one line, `Company, Location` on the next -- no `tabular`/`tabularx`, and (see Known Pitfalls) no `\hfill` either.
...
- **`\hfill`-based right-aligned dates scramble `pdftotext -layout` reading order -- do not reintroduce this.** An earlier version of `\cvheader` right-aligned the dates with `\hfill`. It looked correct on screen, but real-world testing found `pdftotext -layout` can misattribute a date to a *different* entry two lines away once column x-positions shift slightly between headers (confirmed live: an Education entry's date extracted next to the wrong degree). This is worse than a table for ATS purposes because it looks fine visually and only breaks in the text layer. Fixed by putting the date inline right after the title (`Title -- Dates`) instead of right-aligning it -- always re-run the Step 5d-style `pdftotext -layout` check on any edit to `\cvheader`, don't just eyeball the PDF.
- **`\textbullet` as the itemize marker can extract as a `�` (replacement character), not a bullet.** Under `lualatex` + `lmodern`'s Type 1 fallback fonts, `\textbullet` can lack a proper ToUnicode mapping. Fixed by using a literal ASCII `-` (`label=--` in `\setlist`) instead, which always extracts cleanly. Don't switch back to `\textbullet`/other symbol glyphs without re-checking the text extraction.
...
- **`\needspace{N\baselineskip}` is used before every `\cvheader{}`, and this is intentional, not a contradiction of the "no forced page count" design.** Without it, a job's bullet list can split mid-list across a page break (confirmed live: a 3-bullet entry had its 3rd bullet stranded alone on page 2). `\needspace` here only keeps one job/degree block from breaking internally -- it does not force the overall document to 1 or 2 pages, which is still entirely content-driven. Keep the `\needspace` calls; do not remove them to "simplify."
